...
avail = requests.get("https://adc.arm.gov/armlive/livedata/query",
                     params={"user": f"{user}:{token}", "ds": "sgpaerioe1turnC1.c1",
                             "start": "2023-06-11", "end": "2023-06-11", "wt": "json"}).json()
...
```python
files = act.discovery.download_arm_data(user, token, "sgpaerioe1turnC1.c1", start, end)
ds = act.io.arm.read_arm_netcdf(files, keep_variables=['Akernal', 'Sa', 'Sop'],
                                cleanup_qc=True)
```
