...
avail = requests.get("https://adc.arm.gov/armlive/livedata/query",
                     params={"user": f"{user}:{token}", "ds": "sgp180varanal3drucC1.c1",
                             "start": "2011-04-22", "end": "2011-04-22", "wt": "json"}).json()
...
```python
files = act.discovery.download_arm_data(user, token, "sgp180varanal3drucC1.c1", start, end)
ds = act.io.arm.read_arm_netcdf(files, keep_variables=['KB', 'KT', 'Lprecip'],
                                cleanup_qc=True)
```
