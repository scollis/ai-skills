...
avail = requests.get("https://adc.arm.gov/armlive/livedata/query",
                     params={"user": f"{user}:{token}", "ds": "gucxprecipradarcmacppiS2.c1",
...
```python
files = act.discovery.download_arm_data(user, token, "gucxprecipradarcmacppiS2.c1", start, end)
ds = act.io.arm.read_arm_netcdf(files, keep_variables=['DBZ', 'DBZhv', 'NCP'],
                                cleanup_qc=True)
```
