...
avail = requests.get("https://adc.arm.gov/armlive/livedata/query",
                     params={"user": f"{user}:{token}", "ds": "sgp50rwpwindC1.b1",
                             "start": "2001-03-28", "end": "2001-03-28", "wt": "json"}).json()
...
```python
files = act.discovery.download_arm_data(user, token, "sgp50rwpwindC1.b1", start, end)
ds = act.io.arm.read_arm_netcdf(files, keep_variables=["avgint", "azimuth0", "azimuth1"],
                                cleanup_qc=True)
```
