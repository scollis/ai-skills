...
avail = requests.get("https://adc.arm.gov/armlive/livedata/query",
                     params={"user": f"{user}:{token}", "ds": "sgpnoaaaosavgC1.b1",
                             "start": "2017-03-28", "end": "2017-03-28", "wt": "json"}).json()
...
```python
files = act.discovery.download_arm_data(user, token, "sgpnoaaaosavgC1.b1", start, end)
ds = act.io.arm.read_arm_netcdf(files, keep_variables=["Ba_G_Dry_10um_PSAP1W_1", "Ba_G_Dry_10um_PSAP1W_1_N", "Ba_G_Dry_10um_PSAP1W_1_std"],
                                cleanup_qc=True)
```
