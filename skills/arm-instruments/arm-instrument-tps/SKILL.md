...
avail = requests.get("https://adc.arm.gov/armlive/livedata/query",
                     params={"user": f"{user}:{token}", "ds": "nsatpsauxC1.b1",
                             "start": "2014-05-18", "end": "2014-05-18", "wt": "json"}).json()
print(avail["num_found"], avail["total_size"])            # files, bytes

# Downloads into ./nsatpsauxC1.b1/ unless you pass output=
files = act.discovery.download_arm_data(user, token, "nsatpsauxC1.b1", "2014-05-18", "2014-05-18")
assert files, "nothing transferred - ARM Live rate limits with HTTP 429; retry"
ds = act.io.arm.read_arm_netcdf(files, cleanup_qc=True)
print(act.discovery.get_arm_doi("nsatpsauxC1.b1", "2014-05-18", "2014-05-18"))   # cite what you pulled
```
### First look
...
disp.plot("inst_logger_temp", assessment_overplot=True)   # QC-flagged points in red/orange
...
print(ds["qc_amps_max"].attrs.get("flag_meanings", "no flag_meanings"))
mask = ds.qcfilter.get_masked_data("amps_max", rm_assessments=["Bad", "Indeterminate"],
                                  return_mask_only=True)
print(int(mask.sum()), "of", mask.size, "points flagged")

# NaN-fill in place. ARM b1 files use Bad/Indeterminate, VAPs often use
# Incorrect/Suspect - pass every name you might meet.
ds.qcfilter.datafilter(variables=["amps_max", "amps_mean", "inst_logger_temp"],
