...
avail = requests.get("https://adc.arm.gov/armlive/livedata/query",
                     params={"user": f"{user}:{token}", "ds": "enaaossmpsC1.b1",
...
```python
files = act.discovery.download_arm_data(user, token, "enaaossmpsC1.b1", start, end)
ds = act.io.arm.read_arm_netcdf(files, keep_variables=["dN_dlogDp", "total_N_conc", "total_SA_conc", "qc_dN_dlogDp", "qc_total_N_conc", "qc_total_SA_conc"],
                                cleanup_qc=True)
```
