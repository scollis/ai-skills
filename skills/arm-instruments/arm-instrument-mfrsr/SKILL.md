...
avail = requests.get("https://adc.arm.gov/armlive/livedata/query",
                     params={"user": f"{user}:{token}", "ds": "sgpmfrsr7nchC1.b1",
                             "start": "2026-09-22", "end": "2026-09-22", "wt": "json"}).json()
print(avail["num_found"], avail["total_size"])            # files, bytes

# Downloads into ./sgpmfrsr7nchC1.b1/ unless you pass output=
files = act.discovery.download_arm_data(user, token, "sgpmfrsr7nchC1.b1", "2026-09-22", "2026-09-22")
assert files, "nothing transferred - ARM Live rate limits with HTTP 429; retry"
ds = act.io.arm.read_arm_netcdf(files, cleanup_qc=True)
print(act.discovery.get_arm_doi("sgpmfrsr7nchC1.b1", "2026-09-22", "2026-09-22"))   # cite what you pulled
```
### First look
...
disp.plot("hemisp_narrowband_filter1", assessment_overplot=True)   # QC-flagged points in red/orange
...
```python
files = act.discovery.download_arm_data(user, token, "sgpmfrsr7nchC1.b1", start, end)
ds = act.io.arm.read_arm_netcdf(files, keep_variables=["hemisp_narrowband_filter1", "hemisp_narrowband_filter2", "hemisp_narrowband_filter3", "qc_hemisp_narrowband_filter1", "qc_hemisp_narrowband_filter2", "qc_hemisp_narrowband_filter3"],
                                cleanup_qc=True)
```
...
print(ds["qc_hemisp_narrowband_filter1"].attrs.get("flag_meanings", "no flag_meanings"))
mask = ds.qcfilter.get_masked_data("hemisp_narrowband_filter1", rm_assessments=["Bad", "Indeterminate"],
                                  return_mask_only=True)
print(int(mask.sum()), "of", mask.size, "points flagged")

# NaN-fill in place. ARM b1 files use Bad/Indeterminate, VAPs often use
# Incorrect/Suspect - pass every name you might meet.
ds.qcfilter.datafilter(variables=["hemisp_narrowband_filter1", "hemisp_narrowband_filter2", "hemisp_narrowband_filter3"],
