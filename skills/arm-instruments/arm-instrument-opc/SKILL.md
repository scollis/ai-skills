...
avail = requests.get("https://adc.arm.gov/armlive/livedata/query",
                     params={"user": f"{user}:{token}", "ds": "bnfminiaosopcM1.b1",
                             "start": "2026-06-29", "end": "2026-06-29", "wt": "json"}).json()
print(avail["num_found"], avail["total_size"])            # files, bytes

# Downloads into ./bnfminiaosopcM1.b1/ unless you pass output=
files = act.discovery.download_arm_data(user, token, "bnfminiaosopcM1.b1", "2026-06-29", "2026-06-29")
assert files, "nothing transferred - ARM Live rate limits with HTTP 429; retry"
ds = act.io.arm.read_arm_netcdf(files, cleanup_qc=True)
print(act.discovery.get_arm_doi("bnfminiaosopcM1.b1", "2026-06-29", "2026-06-29"))   # cite what you pulled
```
### First look
...
print(ds["qc_pump_current_consumption"].attrs.get("flag_meanings", "no flag_meanings"))
mask = ds.qcfilter.get_masked_data("pump_current_consumption", rm_assessments=["Bad", "Indeterminate"],
                                  return_mask_only=True)
print(int(mask.sum()), "of", mask.size, "points flagged")

# NaN-fill in place. ARM b1 files use Bad/Indeterminate, VAPs often use
# Incorrect/Suspect - pass every name you might meet.
ds.qcfilter.datafilter(variables=["pump_current_consumption", "calc_dust_weight", "sample_relative_humidity"],
